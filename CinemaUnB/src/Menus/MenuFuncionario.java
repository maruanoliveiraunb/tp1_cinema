	...
	public static void menu_funcionario(Cinema c) throws Exception {
		...
		while (true) {
			...
			if (tipo.equals("sc")) {
				...
				if (c.salas.existe_sala(nome) == true) {
					System.out
				...
				if (c.filmes.existe_filme(nome) == true) {
					System.out
							.println("\n  Ja existe um filme com esse nome !!! \n ");
				} else {
				...
				sco.mostrardisponiveis(c.b, c.sala_obter(sco.getSalaid())
						.getPoltronas(), c.sala_obter(sco.getSalaid())
						.getFileiras());

				System.out.println("\n");

				if (sco.getVendidos(c.b) < sco.getTotal(c.b)) {
					...
					if (modovalido == true) {
						...
						sco.comprar(c, c.b, tipo4, it,
								c.sala_obter(sco.getSalaid()).getPoltronas(), c
										.sala_obter(sco.getSalaid())
										.getFileiras());
				...
				int disponivel = sco.getTotal(c.b) - sco.getVendidos(c.b);

				System.out.printf("\nDisponiveis : %d", disponivel);
				System.out.printf("\nVendidos : %d", sco.getVendidos(c.b));

				System.out.printf("\n\nInteiras : %d", c.getInteiras());
				System.out.printf("\nMeias : %d", c.getMeias());
